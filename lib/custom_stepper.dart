...
class Stepper extends StatefulWidget {
  ...
  const Stepper({
    ...
    this.elevation,
    this.margin, 
    this.stepStateIndexedColor, 
    this.stepStateEditingColor, 
    this.stepStateCompleteColor,
    this.lineColor,
  ...
  /// custom color for stepper state indexed.
  final Color? stepStateIndexedColor;
  /// custom color for stepper state editing.
  final Color? stepStateEditingColor;
  /// custom color for stepper state completed.
  final Color? stepStateCompleteColor;
  /// custom color for stepper line.
...
class _StepperState extends State<Stepper> with TickerProviderStateMixin {
  ...
  /// Customize the color base on the [StepState]
  Color _circleColor(int index) {
    ...
    if (!_isDark()) {
      ...
      if (widget.steps[index].state == StepState.complete){
        return widget.stepStateCompleteColor ?? colorScheme.onSurface.withOpacity(0.38);
      }
      else if (widget.steps[index].state == StepState.editing){
        return widget.stepStateEditingColor ?? colorScheme.onSurface.withOpacity(0.38);
      }
      else if (widget.steps[index].state == StepState.indexed){
        return widget.stepStateIndexedColor ?? colorScheme.onSurface.withOpacity(0.38);
      }
      else {
        return widget.steps[index].isActive ? colorScheme.primary : colorScheme.onSurface.withOpacity(0.38);
      }
        ...
        if (!_isLast(i))          
          Expanded(
            child: Container(
              ...
              height: 1.2,
              color: widget.steps[i].state == StepState.complete ? widget.lineColor : Colors.grey
            ),
          ),
      ],
    ];
